...
class laserAvoid(Node):
    def __init__(self,name):
        ...
        #declareparam
        self.declare_parameter("linear",0.3) #전진 속도
        self.linear = self.get_parameter('linear').get_parameter_value().double_value
        self.declare_parameter("angular",1.0) # 회전 속도
        self.angular = self.get_parameter('angular').get_parameter_value().double_value
        self.declare_parameter("LaserAngle",45.0) #좌우 부채꼴 판정 각도
        self.LaserAngle = self.get_parameter('LaserAngle').get_parameter_value().double_value
        self.declare_parameter("ResponseDist",0.55) #장애물로 간주할 임계 처리
        self.ResponseDist = self.get_parameter('ResponseDist').get_parameter_value().double_value
        self.declare_parameter("Switch",False) #자동주행 OFF 스위치처럼 동작
    ...
    #조이스틱 콜백, 조이스틱이 활성화되면 자동 회피 중지
    def JoyStateCallback(self, msg):
        if not isinstance(msg, Bool): return
        self.Joy_active = msg.data
        
    #종료 처리 노드 종료 시 정시 Twist 1회 발생
    def exit_pro(self):
